...
export function TaskDialog({ task, initialStatus, busy, onSave, onClose, onDelete }: {
  ...
  async function submit(event: FormEvent) {
    ...
    if (input.title.length > 120) { setError('Keep the title to 120 characters or fewer.'); return; }
    if (input.description.length > 2000) { setError('Keep the description to 2,000 characters or fewer.'); return; }
    setError('');
    try {
  ...
  return (
    <Modal title={task ? 'Edit task' : 'A little step forward'} busy={busy} onClose={onClose}>
      ...
      <form className="task-form" onSubmit={submit} noValidate>
        <fieldset disabled={busy}>
          <div className="form-field">
            ...
            <input id="task-title" autoFocus value={input.title} onChange={event => setInput({ ...input, title: event.target.value })} placeholder="What needs to happen?" required aria-describedby="title-help" />
